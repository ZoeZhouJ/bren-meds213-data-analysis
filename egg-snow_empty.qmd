...
```
# This new code chunk is added for renv virtual environment practice purpose
```{r}
nest_data %>% 
...
snowcover_data_fixed <- snowcover_data %>%
  mutate(Date2 = as_date((Date))) # instead of DMY it converted to YMD
...
snowcover_data_fixed2 <- snowcover_data_fixed %>%
  ...
  mutate(Date2 = ifelse(Date == "8&9 june 06", as_date("08-June-06"), ymd(Date))) #Need to FIX

#snowcover_data_fixed %>% glimpse()
snowcover_data_fixed2 %>% 
  filter(Date == "8&9 june 06")
  #filter(is.na(Date2))
